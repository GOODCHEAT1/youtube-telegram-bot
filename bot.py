...
def download_media(url, media_type="audio"):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        # ✅ Save in MongoDB
        if not downloads_collection.find_one(
            {"video_id": info.get("id"), "type": media_type}
        ):
...
def youtube_search(query, max_results=1):
    ...
    for item in response.get("items", []):
        ...
        results.append(
            (title, f"https://www.youtube.com/watch?v={video_id}", video_id)
        )
    return results
...
def send_file(chat_id, filename, media_type, title):
    try:
        if not os.path.exists(filename):
            bot.send_message(chat_id, "⚠️ File missing, redownloading...")
            return False
...
def start_handler(message):
    ...
    markup.add(
        types.InlineKeyboardButton(
            "Join Channel 🔔", url=f"https://t.me/{CHANNEL_ID.strip('@')}"
        )
    )
    bot.send_photo(
        message.chat.id,
...
def song_handler(message):
    ...
    def process():
        ...
        if cached and os.path.exists(cached.get("file_path", "")):
            ...
            bot.send_message(message.chat.id, f"🎵 Sending cached song: <b>{title}</b>")
        else:
            bot.send_message(message.chat.id, f"🎵 Downloading <b>{title}</b> ...")
            filepath, info = download_media(url, "audio")

        if filepath:
            success = send_file(message.chat.id, filepath, "audio", title)
            if not success:  # Retry if file missing
                filepath, info = download_media(url, "audio")
...
def video_handler(message):
    ...
    def process():
        ...
        if cached and os.path.exists(cached.get("file_path", "")):
            filepath = cached["file_path"]
            bot.send_message(message.chat.id, f"🎬 Sending cached video: <b>{title}</b>")
        else:
            bot.send_message(message.chat.id, f"🎬 Downloading <b>{title}</b> ...")
            filepath, info = download_media(url, "video")

        if filepath:
            success = send_file(message.chat.id, filepath, "video", title)
            if not success:  # Retry if file missing
                filepath, info = download_media(url, "video")
